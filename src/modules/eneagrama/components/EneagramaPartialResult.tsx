    ...
    <div className="min-h-[calc(100vh-5rem)] bg-background px-4 py-12">
      <div className="mx-auto max-w-3xl animate-fade-up">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground text-lg">{respondentName}, confira seu perfil do Eneagrama</p>
        </div>

        {/* MINIMAL FREE RESULT - only dominant type */}
        <div className="bg-card border border-border rounded-2xl overflow-hidden mb-8">
          <div className="gradient-primary p-6 text-center">
            ...
            <p className="text-primary-foreground/80 text-sm">Asa: Tipo {wing} — {wingName}</p>
          </div>
          <div className="p-6">
            <div className="p-4 rounded-xl bg-muted/50">
              <h3 className="font-heading font-semibold text-foreground mb-2">Seus 3 Tipos Predominantes</h3>
              <div className="space-y-2">
                {top3.map((t, i) => (
                  <div key={t.type} className="flex items-center gap-2 text-sm">
                    <span className="font-bold text-foreground">{i + 1}º</span>
                    <span className="w-3 h-3 rounded-full" style={{ backgroundColor: ENEAGRAMA_COLORS[t.type] }} />
                    <span className="text-foreground">Tipo {t.type} — {t.name} ({t.percentage}%)</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="relative mb-8">
          <div className="space-y-3 blur-sm pointer-events-none select-none" aria-hidden>
            {([1, 2, 3, 4, 5, 6, 7, 8, 9] as EneagramaType[]).map((t) => (
              <div key={t} className="flex items-center gap-3 p-2 bg-card rounded-lg border border-border">
                <span className="w-28 text-xs font-medium text-foreground">{t}. {ENEAGRAMA_TYPE_NAMES[t]}</span>
                <div className="flex-1 h-4 rounded-full bg-muted overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: "40%", backgroundColor: ENEAGRAMA_COLORS[t] }} />
                </div>
                <span className="w-8 text-xs font-bold text-foreground text-right">??%</span>
              </div>
            ))}
            <div className="p-5 bg-card rounded-xl border border-border">
              <p className="font-semibold text-foreground mb-1">Motivações e Medos Profundos</p>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex flex-col items-center gap-3 p-6 rounded-2xl bg-background/90 backdrop-blur-sm border border-border shadow-lg">
              ...
              <p className="text-sm text-muted-foreground text-center max-w-[240px]">Desbloqueie para ver o relatório completo do Eneagrama</p>
            </div>
          </div>
        </div>
        ...
        {user && !paywall.isFree && !paywall.isLoading && (
          <div className="bg-card border-2 border-accent/30 rounded-2xl p-8 mb-8">
            <div className="text-center mb-6">
              ...
              <p className="text-muted-foreground text-sm">Desbloqueie agora para uma análise profunda do seu perfil</p>
            </div>
            <div className="grid sm:grid-cols-2 gap-3 mb-6">
              {["Descrição completa da personalidade", "Motivações e medos profundos", "Pontos fortes naturais", "Desafios emocionais", "Influência da asa", "Padrões de comportamento", "Perfil em relacionamentos", "Caminho de desenvolvimento pessoal"].map((item) => (

...
const TEMPERAMENTO_DESCRIPTIONS: Record<TemperamentoType, string> = {
  colerico: "O temperamento Colérico é caracterizado por assertividade, liderança natural, energia e determinação. Pessoas coléricas tendem a assumir papéis de liderança, gostam de estar no comando e tomam decisões rápidas.",
  sanguineo: "O temperamento Sanguíneo é caracterizado por sociabilidade, entusiasmo, otimismo e expressividade. Pessoas sanguíneas são comunicativas, criativas e adoram estar rodeadas de pessoas.",
  melancolico: "O temperamento Melancólico é caracterizado por profundidade, análise, perfeccionismo e sensibilidade. Pessoas melancólicas são detalhistas, organizadas e valorizam qualidade.",
  fleumatico: "O temperamento Fleumático é caracterizado por calma, estabilidade, diplomacia e paciência. Pessoas fleumáticas são pacificadoras naturais, confiáveis e consistentes.",
};
    ...
    <div className="min-h-[calc(100vh-5rem)] bg-background px-4 py-12">
      <div className="mx-auto max-w-3xl animate-fade-up">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground text-lg">{respondentName}, confira seu perfil temperamental</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden mb-8">
          <div className="gradient-primary p-6 text-center">
            ...
            <p className="text-primary-foreground/80 text-sm">com traços de {secondaryLabel}</p>
          </div>
          <div className="p-6">
        ...
        <div className="relative mb-8">
          <div className="space-y-3 blur-sm pointer-events-none select-none" aria-hidden>
            {temperamentos.map((t) => (
              <div key={t} className="flex items-center gap-3 p-3 bg-card rounded-xl border border-border">
                <span className="w-28 text-sm font-medium text-foreground">{TEMPERAMENTO_LABELS[t]}</span>
                <div className="flex-1 h-6 rounded-full bg-muted overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: "55%", backgroundColor: TEMPERAMENTO_COLORS[t] }} />
                </div>
                <span className="w-10 text-sm font-bold text-foreground text-right">??%</span>
              </div>
            ))}
            <div className="p-5 bg-card rounded-xl border border-border">
              <p className="font-semibold text-foreground mb-1">Análise Temperamental Completa</p>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex flex-col items-center gap-3 p-6 rounded-2xl bg-background/90 backdrop-blur-sm border border-border shadow-lg">
              ...
              <p className="text-sm text-muted-foreground text-center max-w-[240px]">Desbloqueie para ver o relatório completo do Temperamento</p>
            </div>
          </div>
        </div>
        ...
        {user && !paywall.isFree && !paywall.isLoading && (
          <div className="bg-card border-2 border-accent/30 rounded-2xl p-8 mb-8">
            <div className="text-center mb-6">
              ...
              <p className="text-muted-foreground text-sm">Desbloqueie agora para uma análise temperamental profunda e personalizada</p>
            </div>
            <div className="grid sm:grid-cols-2 gap-3 mb-6">
              {["Interpretação completa do perfil", "Traços psicológicos principais", "Padrões emocionais detalhados", "Pontos fortes naturais", "Desafios comportamentais", "Perfil em relacionamentos", "Perfil profissional", "Plano de evolução pessoal"].map((item) => (
